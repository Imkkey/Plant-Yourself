...
public class PlayerController : NetworkBehaviour
{
    ...
    public override void Spawned()
    {
        ...
        Debug.Log($"[PlayerController Debug] Spawned! Имя сцены: {UnityEngine.SceneManagement.SceneManager.GetActiveScene().name}, Auth: {(HasInputAuthority ? "Input " : "")}{(HasStateAuthority ? "State" : "")}");
        ...
        if (inMainMenu)
        {
            ...
            // Также отключаем весь UI игрока (инвентарь, никнеймы, прицел)
            Canvas[] canvases = GetComponentsInChildren<Canvas>(true);
            foreach (var c in canvases) if (c != null) c.enabled = false;

            // И выключаем системы, чтобы они не реагировали на кнопки в меню
            if (_inventory != null) _inventory.enabled = false;
            if (_grower != null) _grower.enabled = false;
